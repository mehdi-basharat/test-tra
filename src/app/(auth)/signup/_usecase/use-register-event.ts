...
const useRegisterEvent = () => {
    ...
    onSuccess: async (data, variables) => {
      if (data.data.success) {
        const response = await signIn('credentials', {
          ...
          password: variables?.password || variables?.email,
          redirect: false,
        });
  ...
  const onRegister = useCallback(
    (params: SignupDataType) => {
      ...
      let first_name_apple_temp = '';
      let last_name_apple_temp = '';

      if (Object.keys(dataParse)?.length) {
        const [first_name_apple, ...lastParts] = (
          dataParse?.user?.providerData?.[0]?.displayName || dataParse?.user?.providerData?.[1]?.displayName
        )?.split(' ');
        const last_name_apple = lastParts.join(' ');

        first_name_apple_temp = first_name_apple;
        last_name_apple_temp = last_name_apple;
      }
      ...
      const dataObj = {
        first_name: first_name_apple_temp || firstName,
        last_name: last_name_apple_temp || lastName,
        ...utm,
        ...formData,
      };
